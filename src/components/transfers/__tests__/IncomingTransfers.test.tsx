...
describe('IncomingTransfers', () => {
  ...
  it('IncomingTransfers should fire POST /transfers/:id/accept, show success toast, and remove row when Accept is clicked', async () => {
    const user = userEvent.setup()
    server.use(
      http.get('/api/transfers/incoming', () =>
        HttpResponse.json([
    ...
    // After invalidation the list refetches; the default handler returns one item but now with accepted status
  })
