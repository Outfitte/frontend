...
describe('SharedOutfitDetailPage', () => {
  beforeEach(() => {
    server.use(
        ...
        HttpResponse.json({
          items: [
            mockSharedItem({
              ...
              photos: [mockPhoto({ id: 'photo-i-001', media_key: 'uploads/item-photo.jpg', position: 0 })],
          ...
          outfits: [
            mockSharedOutfit({
              ...
              items: [
                mockOutfitItem({ outfit_id: OUTFIT_ID, item_id: ITEM_ID_SHARED, position: 1 }),
                mockOutfitItem({ outfit_id: OUTFIT_ID, item_id: ITEM_ID_UNSHARED, position: 2 }),
              ],
              photos: [
      ...
      http.get('/api/outfits/:id/logs', () =>
        HttpResponse.json([
          mockOutfitLog({ id: 'outfitlog-001', outfit_id: OUTFIT_ID, worn_on: '2026-03-15', notes: 'Great day' }),
        ])
      )
    )
  })
  ...
  it('SharedOutfitDetailPage should render outfit name and notes as heading', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Autumn Layers' })).toBeInTheDocument()
  ...
  it('SharedOutfitDetailPage should render photo gallery with main photo and thumbnail strip', async () => {
    ...
    expect(screen.getByTestId('outfit-main-photo')).toHaveAttribute('src', '/media/uploads/outfit-photo-001.jpg')
  })
  ...
  it('SharedOutfitDetailPage should show photo placeholder when outfit has no photos', async () => {
    server.use(
      http.get('/api/shares/with-me', () =>
        HttpResponse.json({
          items: [],
          outfits: [mockSharedOutfit({ id: OUTFIT_ID, name: 'Autumn Layers', photos: [] })],
          locations: [],
        })
      )
    )
  ...
  it('SharedOutfitDetailPage should link items in grid to /shared/items/:id when item is in shared-with-me list', async () => {
    ...
    expect(linkedItem.querySelector('a')).toHaveAttribute('href', `/shared/items/${ITEM_ID_SHARED}`)
  })
  ...
  it('SharedOutfitDetailPage should render unclickable placeholder for items not in shared-with-me list', async () => {
    ...
    expect(screen.getByTestId('outfit-item-unlinked').querySelector('a')).toBeNull()
  })
  ...
  it('SharedOutfitDetailPage should switch to next photo when Next arrow is clicked', async () => {
    ...
    await screen.findByRole('heading', { name: 'Autumn Layers' })
    expect(screen.getByTestId('outfit-main-photo')).toHaveAttribute('src', expect.stringContaining('outfit-photo-001'))

    await user.click(screen.getByRole('button', { name: /next photo/i }))

    expect(screen.getByTestId('outfit-main-photo')).toHaveAttribute('src', expect.stringContaining('outfit-photo-002'))
  })
  ...
  it('SharedOutfitDetailPage should display Untitled outfit when outfit name is null', async () => {
    server.use(
      http.get('/api/shares/with-me', () =>
        HttpResponse.json({
          items: [],
          outfits: [mockSharedOutfit({ id: OUTFIT_ID, name: null, photos: [] })],
          locations: [],
        })
      )
    )
    ...
    expect(await screen.findByRole('heading', { name: 'Untitled outfit' })).toBeInTheDocument()
  })
  ...
  it('SharedOutfitDetailPage should render linked item with no photo as placeholder when shared item has no photos', async () => {
    server.use(
      http.get('/api/shares/with-me', () =>
        HttpResponse.json({
          items: [
            mockSharedItem({ id: ITEM_ID_SHARED, name: 'Shared Coat', photos: [] }),
          ],
          outfits: [
            mockSharedOutfit({
              id: OUTFIT_ID,
              name: 'Autumn Layers',
              items: [mockOutfitItem({ outfit_id: OUTFIT_ID, item_id: ITEM_ID_SHARED, position: 1 })],
              photos: [],
            }),
          ],
          locations: [],
  ...
  it('SharedOutfitDetailPage should show loading skeleton while shared-with-me data is fetching', () => {
    ...
    expect(screen.getByTestId('shared-outfit-detail-skeleton')).toBeInTheDocument()
  })
})
