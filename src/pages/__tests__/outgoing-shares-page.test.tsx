...
import { mockShareView, mockItem, mockOutfit, mockLocation } from '@/test/mocks/fixtures'
...
describe('OutgoingSharesPage', () => {
  ...
  it('OutgoingSharesPage should list shares grouped by target_type in Items and Outfits sections', async () => {
    server.use(
      http.get('/api/shares', () =>
        HttpResponse.json([
          mockShareView({ id: 'share-001', target_type: 'item', target_id: 'item-001' }),
          mockShareView({ id: 'share-002', target_type: 'outfit', target_id: 'outfit-001' }),
          mockShareView({ id: 'share-003', target_type: 'location', target_id: 'loc-001' }),
        ])
      ),
      http.get('/api/items', () => HttpResponse.json([mockItem({ id: 'item-001', name: 'Blue Denim Jacket' })])),
      http.get('/api/outfits', () => HttpResponse.json([mockOutfit({ id: 'outfit-001', name: 'Casual Friday' })])),
      http.get('/api/locations', () => HttpResponse.json([mockLocation({ id: 'loc-001', label: 'Main Closet' })]))
    )
    renderPage()
  ...
  it('OutgoingSharesPage should fall back to target_id when target is not found in lookup', async () => {
    server.use(
      http.get('/api/shares', () =>
        HttpResponse.json([
          mockShareView({ id: 'share-001', target_type: 'item', target_id: 'item-deleted' }),
        ])
      ),
